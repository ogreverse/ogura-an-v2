      ...
      const messages = [
        {
          role: "system",
          content: `
        以下のワードと文脈を参考に、ワード、意味（500文字以下で、である調の詳細な説明、略語の場合は分解した言葉を頭に含める）、タグ（カテゴリ分けしやすいタグを、カンマ区切りで１〜３個記載する）、使い方、単語を問う問題文、意味を問う問題文（4つの選択式の穴埋め問題。意味として重要な単語を隠す。）,意味を問う問題文の回答（カンマ区切りで、一番初めを正解とし、以降がその他の選択肢）、の7つの項目を含むJSONを返却してください。
        返却値は必ずJSONだけにしてください。
        JSONのキーは例に合わせてください。
